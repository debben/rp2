...
class TestODSOutputDiff(AbstractTestODSOutputDiff):  # pylint: disable=too-many-public-methods
    ...
    def setUpClass(cls) -> None:
        ...
        for method in AbstractTestODSOutputDiff.METHODS:
            ...
            AbstractTestODSOutputDiff._generate(cls.output_dir, test_name="test_hifo3", config="test_data", method=method)
    ...
    def test_test_hifo3_rp2_full_report(self) -> None:
        for method in self.METHODS:
            self._compare(output_dir=self.output_dir, test_name="test_hifo3", method=method, output_plugin=OutputPlugins.RP2_FULL_REPORT)

    def test_test_hifo3_tax_report_us(self) -> None:
        for method in self.METHODS:
            self._compare(output_dir=self.output_dir, test_name="test_hifo3", method=method, output_plugin=OutputPlugins.TAX_REPORT_US)
